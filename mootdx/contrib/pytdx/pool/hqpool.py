...
class TdxHqPool_API(object):
    ...
    def perform_reflect(self, api_obj):
        ...
        method_names = [
            attr for attr in dir(api_obj) if callable(getattr(api_obj, attr))
        ]
        for method_name in method_names:
            log.debug("testing attr %s" % method_name)
            if method_name[:
                           3] == 'get' or method_name == "do_heartbeat" or method_name == 'to_df':
...
if __name__ == '__main__':
    ...
    print("send api call to primary ip %s, %s" %
          (str(primary_ip), str(hot_backup_ip)))
