    ...
    def get_k_data(self, code, start_date, end_date):
        ...
        def __select_market_code(code):
            ...
            if code[0] in ['5', '6', '9'] or code[:3] in [
                    "009", "126", "110", "201", "202", "203", "204", "113"
            ]:
                return 1
        ...
        data = pd.concat([
            self.to_df(
                self.get_security_bars(9, __select_market_code(code), code,
                                       (9 - i) * 800, 800)) for i in range(10)
        ],
                         axis=0)
...
if __name__ == '__main__':
    ...
    if api.connect('101.227.73.20', 7709):
        ...
        pprint.pprint(data)
        log.info("查询历史分时行情")
        data = api.get_history_minute_time_data(TDXParams.MARKET_SH, '600300',
                                                20161209)
        pprint.pprint(data)
        log.info("查询分时成交")
        ...
        pprint.pprint(data)
        log.info("查询历史分时成交")
        data = api.get_history_transaction_data(TDXParams.MARKET_SZ, '000001',
                                                0, 10, 20170209)
        pprint.pprint(data)
        log.info("查询公司信息目录")
